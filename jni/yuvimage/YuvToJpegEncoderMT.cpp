...
// Unsharp mask sharpening + mild contrast enhancement on the luma plane.
// Applied to every saved photo to compensate for a soft lens.
// Blur = two-pass separable box filter (radius 2, 5-tap), then
// result = src + amount*(src - blur), plus a slight contrast lift.
static void sharpen_luma(uint8_t* y, int width, int height, int stride) {
    int i, r;
    int sum;
    if (width < 8 || height < 8)
        return;
    ...
    // Pass 1: horizontal box filter (radius 2) with running sum -> tmp
    for (r = 0; r < height; r++) {
        uint8_t* row = y + (size_t)r * stride;
        uint8_t* out = tmp + (size_t)r * width;
        int x;
        for (x = 0; x < width; x++) {
            int c0 = x - 2 < 0 ? 0 : x - 2;
            int c1 = x + 2 > width - 1 ? width - 1 : x + 2;
            out[x] = (uint8_t)((row[c0] + row[c0 + 1] + row[x] + row[c1 - 1] + row[c1]) / 5);
        }
    }

    // Pass 2: vertical box filter (radius 2) + unsharp + contrast, write back in place
    for (r = 0; r < height; r++) {
        int t0 = r - 2 < 0 ? 0 : r - 2;
        int t1 = r + 2 > height - 1 ? height - 1 : r + 2;
        uint8_t* row0 = tmp + (size_t)t0 * width;
        uint8_t* row1 = tmp + (size_t)(t0 + 1) * width;
        uint8_t* rowm = tmp + (size_t)r * width;
        uint8_t* row3 = tmp + (size_t)(t1 - 1) * width;
        uint8_t* row4 = tmp + (size_t)t1 * width;
        uint8_t* cur = y + (size_t)r * stride;
        for (i = 0; i < width; i++) {
            int blur = (row0[i] + row1[i] + rowm[i] + row3[i] + row4[i]) / 5;
            int src = cur[i];
            int detail = src - blur;      // high-frequency residual
            int v = src + detail;         // unsharp: amount = 1.0
            int d = v - 128;              // mild contrast lift
            v = 128 + (d * 112) / 100;
            if (v < 0) v = 0;
            else if (v > 255) v = 255;
            cur[i] = (uint8_t)v;
...
boolean YuvToJpegEncoderMT_encode(JNIEnv* env, jobject jstream, jbyteArray jstorage, uint8_t* inYuv, int width,
	...
	// Sharpen the luma plane of every saved frame (NV21 only) to
	// compensate for a soft lens.
	if (format == ImageFormat_NV21 && offsets[0] >= 0 && strides[0] > 0)
		sharpen_luma((uint8_t*)inYuv + offsets[0], width, height, strides[0]);
